...
const features = [
  ...
  },
  {
    ...
    description:
      "Que conecta as pessoas e transforma o ambiente em pura diversão, emoção e nostalgia e que não deixa ninguém parado.",
  },
  {
    icon: Heart,
...
const WhoHiresSection = () => {
  return (
    <section id="diferencial" className="relative py-16 lg:py-20 overflow-hidden">
      {/* Parallax Background */}
      <div
        className="absolute inset-0 parallax-bg"
        style={{
          backgroundImage: "url(/infraestrutura-palco-som-iluminacao.webp)",
      ...
      <div className="relative z-10 container mx-auto px-6">
        <div className="text-center mb-16">
          {/* 1. O H2 PARA O GOOGLE (Invisível) - Destacando os diferenciais e estrutura para B2B e Casamentos */}
          <h2 className="sr-only">
            Diferenciais da Banda: Alta Performance e Infraestrutura para Eventos Corporativos e Casamentos
          </h2>

          {/* 2. O TÍTULO VISUAL (Agora como <p> mantendo o design intacto) */}
          <p className="heading-display text-5xl md:text-6xl lg:text-7xl text-foreground mb-4">
            QUEM NOS <span className="neon-pink-text">CONTRATA</span> PROCURA
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {features.map((feature, index) => (
            <article
              key={feature.title}
              className="glass-card p-8 lg:p-10 rounded-2xl hover-lift group"
              style={{
            ...
            >
              <div className="flex items-start gap-6">
                ...
                <div>
                  <h3 className="heading-display text-2xl md:text-3xl text-foreground mb-3">{feature.title}</h3>
                  <p className="text-body text-lg">{feature.description}</p>
                </div>
              </div>
            </article>
          ))}
